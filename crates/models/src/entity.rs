...
impl Entity {
  ...
  pub fn new(
    ...
    metadata.insert("index_fields".to_string(), serde_json::json!(Self::INDEX_FIELDS));
  ...
  pub fn from_node(
    ...
    let mut entity = Self::new(node_name, Some(entity_type_id), node_description, dataset_id);

    entity.base.set_metadata("original_node_id", serde_json::json!(node_id.into()));
...
mod tests {
  ...
  fn test_entity_from_node() {
    let type_id = Uuid::new_v4();
    let entity = Entity::from_node("techcorp_1", "TechCorp", "A technology company", type_id, None);
