...
# Telepathic - Embedding Crate

Neural code embeddings for source retrieval and search.

- **Local (default):** [CodeRankEmbed](https://huggingface.co/nomic-ai/CodeRankEmbed) (137M, 768-d) via ONNX Runtime — auto-downloads INT8 weights
- **HTTP (opt-in `voyage`):** [voyage-code-3](https://docs.voyageai.com/) (1024-d)
- **Index:** pure-Rust HNSW (cosine); fused search blends `0.7 * neural + 0.3 * semantic`
- **Documents:** tree-sitter / `telepathic-core` definitions (signature metadata + body chunks, 256/32)

```rust
use std::sync::Arc;
use telepathic_embedding::{
  CodeSearcher, EmbeddingConfig, EmbeddingEngine, MockEmbeddingEngine, documents_from_source,
};

# async fn example() -> Result<(), Box<dyn std::error::Error>> {
let engine: Arc<dyn EmbeddingEngine> = Arc::new(MockEmbeddingEngine::default());
let mut searcher = CodeSearcher::new(engine)?;
// searcher.index_documents(&docs).await?;
// let hits = searcher.search("auth middleware", 10, true).await?;
# Ok(())
# }
```

Env: `EMBEDDING_PROVIDER` (`onnx`|`voyage`|`mock`), `MOCK_EMBEDDING`, `EMBEDDING_MODEL`,
`VOYAGE_API_KEY` / `EMBEDDING_API_KEY`.

Algorithmic TF-IDF / RI / MinHash remain in `telepathic-semantic` (static nomic int8 table ≠ full neural).
...
```bash
cargo add telepathic-embedding
```

## Building

Run `nx build native-embedding` to build the library.

## Running unit tests

Run `nx test native-embedding` to execute the unit tests via [Vitest](https://vitest.dev/).
