...
use crate::{
  types::{
    ...
    binding_output::{
      ...
      BindingGetSettingsOutput, BindingListProjectsOutput, BindingListRepositoriesOutput,
      BindingQueryGraphOutput, BindingReadGraphOutput, BindingSearchGraphOutput,
      BindingTraceGraphOutput, BindingWriteGraphOutput,
    },
  },
  utils::map_sdk_result,
};
use telepathic_sdk::Telepathic;
...
impl BindingSDK {
  ...
  pub fn new(options: BindingOptions) -> PyResult<Self> {
    let inner = Telepathic::new(options.into_core())
  ...
  pub async fn get_settings(&mut self) -> PyResult<Py<PyAny>> {
    binding_result_to_py(map_sdk_result(
      self.inner.get_settings().map(BindingGetSettingsOutput::from),
    ))
  }

  pub async fn get_session(&mut self) -> PyResult<Py<PyAny>> {
    binding_result_to_py(map_sdk_result(
      self.inner.get_session().map(BindingGetSessionOutput::from),
    ))
  }

  pub async fn get_schema(&mut self) -> PyResult<Py<PyAny>> {
    binding_result_to_py(map_sdk_result(self.inner.get_schema().map(BindingGetSchemaOutput::from)))
  }

  pub async fn list_repositories(&mut self) -> PyResult<Py<PyAny>> {
    binding_result_to_py(map_sdk_result(
      self.inner.list_repositories().map(BindingListRepositoriesOutput::from),
    ))
  }
  ...
  pub async fn read_graph(&mut self, input: BindingReadGraphInput) -> PyResult<Py<PyAny>> {
    binding_result_to_py(map_sdk_result(
      self.inner.read_graph(input.into()).map(BindingReadGraphOutput::from),
    ))
  }
  ...
  pub async fn trace_graph(&mut self, input: BindingTraceGraphInput) -> PyResult<Py<PyAny>> {
    binding_result_to_py(map_sdk_result(
      self.inner.trace_graph(input.into()).map(BindingTraceGraphOutput::from),
    ))
  }

  pub async fn export_okf(&mut self, input: BindingExportOkfInput) -> PyResult<Py<PyAny>> {
    binding_result_to_py(map_sdk_result(
      self.inner.export_okf(input.into()).map(BindingExportOkfOutput::from),
    ))
  }
